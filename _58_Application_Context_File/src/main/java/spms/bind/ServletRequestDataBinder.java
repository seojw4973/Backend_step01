...
public class ServletRequestDataBinder {
	...
	public static Object bind(
		...
		// request로부터 모든 parameter의 이름을 얻는다.
		...
		// 브라우저가 보내온 파라미터 갯수만큼 반복
		for(String paramName : paramNames) {
			// parameter 값에 대응되는 setter를 찾아서 리턴한다.
			// ex) no : setNo, name : setName
			m = findSetter(dataType, paramName);
			if(m != null) {
				// 메서드 정보를 가지고 메서드를 호출
				// dataObject로부터 m에 저장된 setter 메서드를 호출(invoke)해서 값을 저장한다. 
	...
	private static Method findSetter(Class<?> type, String name) {
		...
		for(Method m : methods) {
			...
			// 비교를 쉽게 하기 위해 이름을 모두 소문자로 변환해서 비교한다.
			/* 메소드 이름의 setNo이고, name 변수에 "no"가 저장되어 있다면
			 * setNo에서 프로퍼티명(propName)은 No이고
			 * No를 모두 소문자로 변경하면 no가 되고
			 * 이 이름과 name에 있는 이름이 같다면
			 * name에 대한 setter 메서드인 setNo 메서드를 찾은 것이다.
			 * 찾았으면 메서드 정보를 반환한다.
			 */
